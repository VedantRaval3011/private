...
async function run() {
  ...
  const startIdx = docXml.indexOf('Trend Analysis of pH at Finished Stage');
  const sectionContent = docXml.substring(startIdx - 1000, startIdx + 150000);
  ...
  fs.writeFileSync('tmp/chart-layout4.txt', out);
}
run();
