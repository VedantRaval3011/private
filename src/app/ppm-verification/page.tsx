...
export default function PpmVerificationPage() {
  ...
  const loadYears = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/requisition/ppm-verification');
  ...
  const loadBaseRowsForSummary = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/requisition/ppm-verification${q}`);
      const data: ApiResponse = await res.json();
  ...
  const displaySummary = useMemo(() => {
    ...
    const coaFound = filtered.filter(r => r.ppmCoaStatus === 'PPM COA Found').length;
    ...
    return {
      ...
      ppmCoaFoundBatches: coaFound,
      ppmCoaMissingBatches: coaMissing,
    };
    ...
    return [
      {
        key: 'total',
        label: 'TOTAL BATCHES',
        sub: selectedYear ? `(${selectedYear})` : '(ALL YEARS)',
        value: displaySummary.totalBatchesCreation,
        drill: 'total_batches' as DrillMode,
        tone: 'neutral' as const,
      },
      {
        key: 'req_found',
        label: 'REQUISITION BATCHES FOUND',
        sub: 'Batches in batch creation + PPM requisition',
        value: displaySummary.requisitionFoundBatches,
        drill: 'requisition_found' as DrillMode,
        tone: 'good' as const,
      },
      {
        key: 'req_missing',
        label: 'REQUISITION BATCHES MISSING',
        sub: 'Batches in batch creation only (no PPM requisition)',
        value: displaySummary.requisitionMissingBatches,
        drill: 'requisition_missing' as DrillMode,
        tone: 'bad' as const,
      },
      {
        key: 'coa_found',
        label: 'PPM COA FOUND',
        sub: 'Among requisition-found batches',
        value: displaySummary.ppmCoaFoundBatches,
        drill: 'ppm_coa_found' as DrillMode,
        tone: 'good' as const,
      },
      {
        key: 'coa_missing',
        label: 'PPM COA MISSING',
        sub: 'Among requisition-found batches',
        value: displaySummary.ppmCoaMissingBatches,
        drill: 'ppm_coa_missing' as DrillMode,
        tone: 'bad' as const,
      },
    ];
    ...
    const searched = q
      ? scoped.filter(r => {
          return (
            ...
            (r.ppmCoaStatus || '').toLowerCase().includes(q)
          );
        })
      : scoped;
  ...
  const coaOk = displaySummary.ppmCoaFoundBatches + displaySummary.ppmCoaMissingBatches === displaySummary.requisitionFoundBatches;

  const drillTitle: Record<DrillMode, string> = {
    total_batches: `All batch creation batches (${selectedYear || 'All Years'})`,
    requisition_found: 'Batches in batch creation and PPM requisition',
    requisition_missing: 'Batches in batch creation only (not on PPM requisition)',
    ppm_coa_found: 'Requisition batches with PPM COA',
    ppm_coa_missing: 'Requisition batches without PPM COA',
  };
      ...
      <header style={{ background: 'linear-gradient(135deg, #0f766e 0%, #115e59 100%)', padding: '1.5rem 0' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 1.5rem' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1.25rem' }}>
            <div>
              <h1 style={{ fontSize: '1.65rem', fontWeight: 800, color: 'white', margin: 0, letterSpacing: '-0.02em' }}>PPM verification</h1>
              <p style={{ color: 'rgba(255,255,255,0.88)', fontSize: '0.9rem', marginTop: '0.35rem', maxWidth: '560px' }}>
                Step-wise batch verification for PPM materials: totals from batch creation and PPM requisition coverage. Click a capsule to open the detail table.
              </p>
            </div>
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '1.5rem' }}>
        ...
        <div style={{ marginTop: '0.75rem', padding: '0.65rem 0.9rem', borderRadius: '12px', border: '1px solid #e2e8f0', background: 'white', color: '#334155' }}>
          <div style={{ fontWeight: 800 }}>
            Requisition check:{' '}
            Found ({displaySummary.requisitionFoundBatches.toLocaleString()}) + Missing ({displaySummary.requisitionMissingBatches.toLocaleString()}) = Total ({displaySummary.totalBatchesCreation.toLocaleString()})
            {reqOk ? ' ✓' : ' (mismatch)'}
          </div>
          <div style={{ marginTop: '0.2rem', fontWeight: 800 }}>
            PPM COA check:{' '}
            COA Found ({displaySummary.ppmCoaFoundBatches.toLocaleString()}) + COA Missing ({displaySummary.ppmCoaMissingBatches.toLocaleString()}) = Requisition Found ({displaySummary.requisitionFoundBatches.toLocaleString()})
            {coaOk ? ' ✓' : ' (mismatch)'}
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                ...
                <th onClick={() => toggleSort('ppmCoaStatus')} style={{ padding: '0.85rem 0.9rem', textAlign: 'left', cursor: 'pointer', color: '#0f172a', fontWeight: 900 }}>
                  COA Status {sortKey === 'ppmCoaStatus' ? (sortDir === 'asc' ? '↑' : '↓') : ''}
                </th>
              </tr>
            </thead>
            <tbody>
              {loadingData ? (
                ...
                filteredSortedRows.map((r, idx) => {
                  ...
                  const sCoa = statusBadgeStyle(r.ppmCoaStatus);
                  return (
